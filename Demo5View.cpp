...
void CDemo5View::DrawBezier()
{
	CDC *pDC=GetDC();
	CPen NewPen,*pOldPen;
	NewPen.CreatePen(PS_SOLID,1,RGB(0,0,255));//ÇúÏßÑÕÉ«ÎªÀ¶É«
...
void CDemo5View::DrawCtrlPolygon()
{
	...
	pOldBrush=(CBrush*)pDC->SelectStockObject(GRAY_BRUSH);//»ÒÉ«ÊµÐÄÔ²»æÖÆ¿ØÖÆµã
	for(int i=0;i<=n;i++)
	{
		if(0==i)
		{
...
void CDemo5View::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	if(bFlag)
	{
		...
		if(CtrlPointNum<N_MAX_POINT-1)//N_MAX_POINTÎª¿ØÖÆµã¸öÊýµÄ×î´óÖµ
			CtrlPointNum++;
		else
			bFlag=FALSE;
		n=CtrlPointNum-1;
...
void CDemo5View::B3Curves()
{
	...
	NewPen.CreatePen(PS_SOLID,1,RGB(0,0,255));//ÇúÏßÑÕÉ«ÎªÀ¶É«
	pOldPen=pDC->SelectObject(&NewPen);	
	q.x=Round((P[0].x+4.0*P[1].x+P[2].x)/6.0);//t£½0µÄÆðµãx×ø±ê
	q.y=Round((P[0].y+4.0*P[1].y+P[2].y)/6.0);//t£½0µÄÆðµãy×ø±ê
	
	pDC->MoveTo(q);
	for(double t=0;t<=1;t+=0.01)
		{
			F03=(-t*t*t+3*t*t-3*t+1)/6;//¼ÆËãF0,3(t)
			F13=(3*t*t*t-6*t*t+4)/6;//¼ÆËãF1,3(t)
			F23=(-3*t*t*t+3*t*t+3*t+1)/6;//¼ÆËãF2,3(t)
			F33=t*t*t/6;//¼ÆËãB3,3(t)
